...
# --- MODIFICATION: Removed bravado, EsiClientProvider ---
...
from .helpers import is_fleet_commander, _update_fleet_structure
# --- NEW IMPORTS ---
from . import esi
...
def api_fc_manage_waitlist(request):
    ...
    if action == 'close':
        # ... (No ESI calls in this block, no changes needed) ...
        ...
        try:
            ...
            # 2. Make ESI call to get fleet info
            logger.debug(f"Getting ESI fleet info for {fc_character.character_name}")
            fleet_info = esi.make_esi_call(
                esi_client.client.Fleets.get_characters_character_id_fleet,
            ...
            # 3. Check if character is the fleet boss
        ...
        except (EsiTokenAuthFailure, EsiScopeMissing, EsiForbidden, EsiException) as e:
            logger.error(f"Error linking fleet: {e.message}", exc_info=True)
            return JsonResponse({"status": "error", "message": e.message}, status=e.status_code)
        # --- END NEW ---
        except Exception as e:
            logger.error(f"Non-ESI error linking fleet: {e}", exc_info=True)
            return JsonResponse({"status": "error", "message": f"An error occurred: {str(e)}"}, status=500)
...
def api_get_fleet_structure(request):
    ...
    from the database.
    (No ESI calls here, no changes needed)
    """
    # ... (code redacted for brevity) ...
...
def api_get_fleet_members(request):
    ...
    try:
        ...
        esi_members = esi.make_esi_call(
            esi_client.client.Fleets.get_fleets_fleet_id_members,
            character=fc_character,
        ...
        if missing_char_ids:
            ...
            try:
                # --- MODIFICATION: Use make_esi_call ---
                names_response = esi.make_esi_call(
                    esi_client.client.Universe.post_universe_names,
        ...
        # ... (Ship counting logic remains the same) ...
        # ... (code redacted for brevity) ...
        ...
        # ... (Final processing logic remains the same) ...
        # ... (code redacted for brevity) ...
        final_wings_list = []
    ...
    # --- NEW: Catch custom exceptions ---
        ...
        logger.error(f"Error getting fleet members: {e.message}", exc_info=True)
        return JsonResponse({"status": "error", "message": e.message}, status=e.status_code)
    # --- END NEW ---
    except Exception as e:
        logger.error(f"Non-ESI error getting fleet members: {e}", exc_info=True)
        return JsonResponse({"status": "error", "message": f"An error occurred: {str(e)}"}, status=500)
...
def api_save_squad_mappings(request):
    ...
    try:
        ...
        fc_character = fleet.fleet_commander
        esi_client = esi.get_esi_client()
        # Token will be fetched by make_esi_call
        # --- END MODIFICATION ---
        ...
        for wing_info in wing_data:
            ...
            if db_wing and db_wing.name != new_name:
                ...
                # --- MODIFICATION: Use make_esi_call ---
                esi.make_esi_call(
                    esi_client.client.Fleets.put_fleets_fleet_id_wings_wing_id,
                    character=fc_character,
        ...
        for squad_info in squad_data:
            ...
            if db_squad:
                if db_squad.name != new_name:
                    ...
                    # --- MODIFICATION: Use make_esi_call ---
                    esi.make_esi_call(
                        esi_client.client.Fleets.put_fleets_fleet_id_squads_squad_id,
                        character=fc_character,
                        required_scopes=['esi-fleets.write_fleet.v1'],
                        fleet_id=fleet.esi_fleet_id,
                        squad_id=squad_id,
        ...
        # _update_fleet_structure now uses the new ESI service
        _update_fleet_structure(fleet)
        
        # ... (Get new structure logic remains the same) ...
        # ... (code redacted for brevity) ...
        wings = FleetWing.objects.filter(fleet=fleet).prefetch_related('squads')
        available_categories = [
            {"id": choice[0], "name": choice[1]}
    ...
    # --- NEW: Catch custom exceptions ---
    except (EsiTokenAuthFailure, EsiScopeMissing, EsiForbidden, EsiNotFound, EsiException) as e:
        logger.error(f"Error saving squad mappings: {e.message}", exc_info=True)
        return JsonResponse({"status": "error", "message": e.message}, status=e.status_code)
    # --- END NEW ---
    except Exception as e:
        logger.error(f"Non-ESI error saving squad mappings: {e}", exc_info=True)
        return JsonResponse({"status": "error", "message": f"An error occurred: {str(e)}"}, status=500)
...
def api_fc_invite_pilot(request):
    ...
    try:
        ...
        # ... (Role/squad finding logic remains the same) ...
        # ... (code redacted for brevity) ...
        role = "squad_member"
        wing_id = None
        squad_id = None
        ...
        # --- MODIFICATION: Use make_esi_call ---
        esi.make_esi_call(
            esi_client.client.Fleets.post_fleets_fleet_id_members,
            character=fc_character,
            required_scopes=['esi-fleets.write_fleet.v1'],
            fleet_id=fleet.esi_fleet_id,
    ...
    # --- NEW: Catch custom exceptions ---
        ...
        logger.error(f"Error inviting pilot for fit {fit_id}: {e.message}", exc_info=True)
        return JsonResponse({"status": "error", "message": f"ESI Error: {e.message}"}, status=e.status_code)
    # --- END NEW ---
    except Exception as e:
        logger.error(f"Non-ESI error inviting pilot for fit {fit_id}: {e}", exc_info=True)
        return JsonResponse({"status": "error", "message": f"An error occurred: {str(e)}"}, status=500)
...
def api_fc_create_default_layout(request):
    ...
    try:
        ...
        current_wings = esi.make_esi_call(
        ...
        for wing_def in DEFAULT_LAYOUT:
            # ... (Wing/squad creation logic remains the same) ...
            # ... (It now uses make_esi_call internally via helper functions) ...
            # ... (code redacted for brevity) ...
            squad_index = 0
            ...
            if esi_wing:
                ...
                esi.make_esi_call(
                    esi_client.client.Fleets.put_fleets_fleet_id_wings_wing_id,
                    character=fc_character, required_scopes=['esi-fleets.write_fleet.v1'],
                    fleet_id=fleet_id, wing_id=wing_id, naming={'name': wing_name}
                )
            else:
                logger.debug(f"Creating new wing, renaming to '{wing_name}'")
                new_wing_op = esi.make_esi_call(
                    esi_client.client.Fleets.post_fleets_fleet_id_wings,
                    character=fc_character, required_scopes=['esi-fleets.write_fleet.v1'],
                    fleet_id=fleet_id
                )
            ...
            for squad_def in wing_def['squads']:
                ...
                if esi_squad:
                    ...
                    esi.make_esi_call(
                        esi_client.client.Fleets.put_fleets_fleet_id_squads_squad_id,
                        character=fc_character, required_scopes=['esi-fleets.write_fleet.v1'],
                        fleet_id=fleet_id, squad_id=squad_id, naming={'name': squad_name}
                    )
                else:
                    logger.debug(f"  Creating new squad in wing {wing_id}, renaming to '{squad_name}'")
                    new_squad = esi.make_esi_call(
                        esi_client.client.Fleets.post_fleets_fleet_id_wings_wing_id_squads,
                        character=fc_character, required_scopes=['esi-fleets.write_fleet.v1'],
                        fleet_id=fleet_id, wing_id=wing_id
                    )
                ...
                FleetSquad.objects.create(
                    wing=db_wing, squad_id=squad_id, name=squad_name, assigned_category=category
                )
                squad_index += 1
            if squad_index < len(existing_squads):
                for i in range(squad_index, len(existing_squads)):
                    ...
                    FleetSquad.objects.create(
                        wing=db_wing, squad_id=squad_id, name=squad_name, assigned_category=None
                    )
            wing_index += 1
    ...
    # --- NEW: Catch custom exceptions ---
        ...
        logger.error(f"Error creating default layout: {e.message}", exc_info=True)
        return JsonResponse({"status":"error", "message": e.message}, status=e.status_code)
    # --- END NEW ---
    except Exception as e:
        logger.error(f"Non-ESI error creating default layout: {e}", exc_info=True)
        return JsonResponse({"status":"error", "message": f"An error occurred: {str(e)}"}, status=500)
...
def api_fc_refresh_structure(request):
    ...
    try:
        # --- MODIFICATION: _update_fleet_structure now handles all ESI ---
        _update_fleet_structure(fleet)
        ...
        logger.info(f"Fleet structure refreshed for {fleet.id} by {fleet.fleet_commander.character_name}")
        return JsonResponse({"status": "success", "structure": structure})

    except EsiNotFound as e:
        # ... (Auto-close logic remains the same) ...
        # ... (code redacted for brevity) ...
    ...
    # --- NEW: Catch custom exceptions ---
        ...
        logger.error(f"Error refreshing fleet structure: {e.message}", exc_info=True)
        return JsonResponse({"status":"error", "message": e.message}, status=e.status_code)
    # --- END NEW ---
    except Exception as e:
        logger.error(f"Non-ESI error refreshing fleet structure: {e}", exc_info=True)
        return JsonResponse({"status":"error", "message": f"An error occurred: {str(e)}"}, status=500)
...
def api_fc_add_squad(request):
    ...
    # --- NEW: Catch custom exceptions ---
        ...
        logger.error(f"Error adding squad to wing {wing_id}: {e.message}", exc_info=True)
        return JsonResponse({"status":"error", "message": e.message}, status=e.status_code)
    # --- END NEW ---
    except Exception as e:
        logger.error(f"Non-ESI error adding squad to wing {wing_id}: {e}", exc_info=True)
        return JsonResponse({"status":"error", "message": f"An error occurred: {str(e)}"}, status=500)
...
def api_fc_delete_squad(request):
    ...
    try:
        ...
        esi.make_esi_call(
            esi_client.client.Fleets.delete_fleets_fleet_id_squads_squad_id,
            character=fc_character,
            required_scopes=['esi-fleets.write_fleet.v1'],
            fleet_id=fleet.esi_fleet_id,
            squad_id=squad_id
    ...
    # --- NEW: Catch custom exceptions ---
        ...
        logger.error(f"Error deleting squad {squad_id}: {e.message}", exc_info=True)
        return JsonResponse({"status":"error", "message": e.message}, status=e.status_code)
    # --- END NEW ---
    except Exception as e:
        logger.error(f"Non-ESI error deleting squad {squad_id}: {e}", exc_info=True)
        return JsonResponse({"status":"error", "message": f"An error occurred: {str(e)}"}, status=500)
...
def api_fc_add_wing(request):
    ...
    # --- NEW: Catch custom exceptions ---
        ...
        logger.error(f"Error adding wing: {e.message}", exc_info=True)
        return JsonResponse({"status":"error", "message": e.message}, status=e.status_code)
    # --- END NEW ---
    except Exception as e:
        logger.error(f"Non-ESI error adding wing: {e}", exc_info=True)
        return JsonResponse({"status":"error", "message": f"An error occurred: {str(e)}"}, status=500)
...
def api_fc_delete_wing(request):
    ...
    try:
        ...
        esi.make_esi_call(
            esi_client.client.Fleets.delete_fleets_fleet_id_wings_wing_id,
            character=fc_character,
            required_scopes=['esi-fleets.write_fleet.v1'],
            fleet_id=fleet.esi_fleet_id,
            wing_id=wing_id
    ...
    # --- NEW: Catch custom exceptions ---
        ...
        logger.error(f"Error deleting wing {wing_id}: {e.message}", exc_info=True)
        return JsonResponse({"status":"error", "message": e.message}, status=e.status_code)
    # --- END NEW ---
    except Exception as e:
        logger.error(f"Non-ESI error deleting wing {wing_id}: {e}", exc_info=True)
        return JsonResponse({"status":"error", "message": f"An error occurred: {str(e)}"}, status=500)


# ---
# --- MODIFICATION: ALL RULE HELPER VIEWS REMOVED ---
# ---